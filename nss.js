...
function setupChartTradingGame() {
  ...
  window.addEventListener('pointerup', handleChartPressEnd);
  window.addEventListener('pointercancel', handleChartPressEnd);
  window.addEventListener('blur', handleChartPressEnd);
...
function initializeChartGameRound(data) {
  ...
  chartGameState.isHolding = false;
...
function handleChartPressEnd() {
  if (!chartGameState.isHolding) return;
  chartGameState.isHolding = false;
  const currentPrice = getCurrentChartPrice();
  if (!currentPrice) return;
  if (chartGameState.inPosition) {
    const tradeReturn = ((currentPrice - chartGameState.entryPrice) / chartGameState.entryPrice) * 100;
    chartGameState.totalReturnPercent += tradeReturn;
    chartGameState.inPosition = false;
    chartGameState.entryPrice = 0;
  }
  chartGameState.currentReturnPercent = 0;
